"""
Compute the aggregate effects for all names individually.
...
def load_files(folder_name, model_name):
    ...
    for race in racenames:
        ...
        for fname in fnames:
            csv_files = [
                f
                for f in os.listdir(os.path.join(folder_name, model_name, race, fname))
                if "_" + model_name + ".csv" in f and f.endswith("csv")
            ]
            folder_names[race][fname] = csv_files
    return folder_names

def compute_agg_effect(folder_name, model_name, race, name, csv_files):
    ...
    paths = [os.path.join(folder_name, model_name, race, name, f) for f in csv_files]
    woman_files = [
        f
        for f in paths
    ...
    path_name = os.path.join(folder_name, model_name, race, model_name + "_" + race + "_" + name +"_neuron_effects.csv")
...
def main(folder_name="results/20191114_neuron_intervention/", model_name="distilgpt2"):
    # create a dictionary of the form (race, (names, [csv files]))
    data = load_files(folder_name, model_name)

    for race in data:
        print("\nComputing neuron aggregate effect for", race)
        names = data[race]
        for name, csv_files in names.items():
            print("\n"+name+":")
            compute_agg_effect(folder_name, model_name, race, name, csv_files)
